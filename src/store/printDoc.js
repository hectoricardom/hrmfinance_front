...
export const  Print_long_ckech_ = async( ) =>{
  
  var doc =  new jsPDF('p', 'mm', [ 210,  276])
  ...
  const item = Util.getFormStore(_formName) || {};

  //console.log(item)

  let dt = item["date"]?(new Date(item["date"])):(new Date());
  ...
  doc.setFontSize(10);	
  //doc.text(xPos, yguide+7, `${orderId} ${""}`);


  doc.setFontType("bold");
  doc.setFontSize(12);
  doc.setTextColor(3,3,3);
  doc.text(xPos + 15, 34  , `${payto?payto:""}`);
  doc.text(xPos + 165, 33.25  , `${monto?monto.toFixed(2):"0.00" }`);
  doc.text(xPos + 2, 43, `${amountTx}`);

  doc.text(xPos + 141, 24  , `${date}`);

  doc.setFontType("normal");
  doc.setTextColor(3,3,3);
  doc.setFontSize(10);	
  doc.text(xPos + 11, 59  , `${description}`);
  
  

    

  doc.save(`tag_${"orderId"}_${""}_${(new Date()).getTime()}.pdf`);
