

import React, { useEffect, useState } from 'react'
import { useSelector, useDispatch } from 'react-redux'
import loadable from '@loadable/component'



import { withRouter} from 'react-router-dom';

import {  OpenWatchDialog, fetchCities, fetchRateCurrency, fetchSenders, fetchUsers, fetchRemesa, OpenModal, CloseModal } from '../../actions/common'



import * as _Util from '../../store/Util'

import '../_styles.css'

const Icon2 = loadable(() => import('../Icons'))

const MoreInfoButton = loadable(() => import('../MoreInfoButton'))

const ImageButton = loadable(() => import('../ImageButton'))

const ChoiceButton = loadable(() => import('../ChoiceButton'))

const PaymentSlideUp = loadable(() => import('../paymentSlideUp'))

const InputText = loadable(() => import('../InputText'))

const InputAutocomplete = loadable(() => import('../InputAutocomplete'))

const BTN_f = loadable(() => import('../btns_confirm'))


const ModalDate = loadable(() => import('../ModalDate'))


const _formName = 'add_btc_agent';


const useObserveChanges = () => {
  const observeChanges =  useSelector(state => state.observeChanges);
  const dispatch = useDispatch(); 


  const _openMd = (_id, item) => {
    let data = {};
    data['zIndex']=450;
    data['Id']=_id;
    data['observeResize']=true;    
    data['props']={item:item, minHeight: '1vh'};
    data['content']=<PaymentSlideUp />;
    OpenWatchDialog(dispatch,data);
  }

  const _LoadCities= (q) => {
    fetchCities(q,dispatch);
  }

  const _LoadRateCurrency= (q) => {
    fetchRateCurrency(q,dispatch);
  }


  const callBackDate= (f,v) => {
    let dD = new Date(v);
    const _form = _Util.getFormStore(_formName) || {};
    let frm =  _form;
    frm[f] = dD.getTime();
    _Util.updFormStore(_formName,frm)
  }



  const OpenModalDate = (i, callBack) => {
    let data = {};
    const _form = _Util.getFormStore(_formName) || {};
    let  _date = _form && _form[i];
    data['zIndex']=190;
    data['observeResize']=true;
    data['props']={title:"", item:i};
    data['content'] = <ModalDate valueUpdate={(e)=>callBackDate(i,e)} initValue={_date}/>; 
    //console.log(data)
    OpenModal(dispatch,data);
  }
  


  
  return { 
    observeChanges,
    _LoadCities,
    dispatch,
    OpenModalDate,
    _LoadRateCurrency,
    _openMd
  }
}





const useObserveForms = () => {
  const observeForms =  useSelector(state => state.observeForms);
  const dispatch = useDispatch(); 


  const _updFormObs = (q,operation) => {
    dispatch({
      type: 'UPD_KEY_VALUE',
      kv:{key:'observeForms',value:_Util.gen12CodeId()}
    })
  }


 
  return { 
    _updFormObs,
    observeForms
  }
}

  














const BrowseComponent = (props) => {
  const {
    _openMd,
    OpenModalDate,
    dispatch,
    _LoadRateCurrency
  } = useObserveChanges();

  const {
    _updFormObs
  } = useObserveForms();
  


  let _state = _Util.getStore();
  let keys = _Util.getGlobalsKeys()
  _state["keys"] = keys;
  const _form = _Util.getFormStore(_formName) || {};




  const [initialize, setInitialize] = useState(false);

  const [widthScreen, setWidthScreen] = useState(null);

  const [choosePayView, setChoosePayView] = useState(0);
  const [validForm, setIsValidForm] = useState(false);
  const [nextValidForm, setNextValidForm] = useState(false);
  const [formView, setformView] = useState(1);
  const [view, setView] = useState(false);

  
  const searchHash = window.location.search.split('?')[1]?window.location.search.split('?')[1]:null;
  //const typeBrowse = window.location.hash.split('/')[1];


  const router = _Util.parseQuery(searchHash);
  
  let outerWidth = _state["outerWidth"];




  useEffect(() => {  
    if(widthScreen!==outerWidth){
      setWidthScreen(outerWidth);
    } 
    if(!initialize){
      setInitialize(true);
      setTimeout(()=>window.scrollTo(0,0),350);
      setTimeout(()=>setView(true),50);
      window.localStorage.setItem("lng","es");

      let Qry = {
        query:"getRateCurrency"
      };
      _LoadRateCurrency(Qry);


      let Qry2 = {
        query:"getQuerySenderDetails",
        Collection:"Senders"
      };
      fetchSenders(Qry2,dispatch);  
      

      let QryUser = {
        query:"getQueryUsersDetails",
        Collection:"Users"
      };
      fetchUsers(QryUser,dispatch);  

      let frm =  _Util.getFormStore(_formName);
      if(frm && !frm["id"]){
        let initF = {
          amount:100,
          date:(new Date()).getTime()
        }
        _Util.updFormStore(_formName,initF)
        _updFormObs();
      }
    }
  });
  


  const handleInput = (v,f) => {
    let frm =  _form;
    frm[f] = v
    _Util.updFormStore(_formName,frm)
    validateFields()
  }


  
  const vldFlds2 = {
    amount:{reqired: true, number: true,minValue:10, maxValue:5000},
    agentId:{reqired:true},
    agentDestination:{reqired:true},
    date:{reqired:true},
    tasa:{reqired: true, number: true, minValue:1, maxValue:90},
  }


  let fld2Prs = [
    'id',
    "amount",
    "agentId",
    "agentDestination",
    "senderId",
    "receiverId",
    "description",
    "tasa",
    "type",
    "delivery",
    "date"
  ]; 


  const handleInputAutoReceive = (v,f) => {
    let frm =  _form;
    frm[f] = v.id
    _Util.updFormStore(_formName,frm)
    validateFields()
  }


  const validateFields = (v,f) => {  
    let frm =  _Util.getFormStore(_formName);
    let _2Fs = frm || {};     
    let _2s = {};
    _2Fs && fld2Prs.map(fld=>{
      _2s[fld] = _2Fs[fld];
    });
    let vld = vldFlds2;
    if(!_2s["date"]){
      _2s["date"] = (new Date()).getTime();
    };
    var _Valid = _Util.validations(vld,_2s); 
    if(_Valid.valid){
      setIsValidForm(true)
    }else{
      setIsValidForm(false)
    }
    _updFormObs();
  }



 
  const _save = () => {
    let frm =  _Util.getFormStore(_formName); 
    let _2Fs = frm || {};     
    const _2s = {};
    _2Fs && fld2Prs.map(fld=>{
      _2s[fld] = _2Fs[fld];
    })    
    let vld = vldFlds2;
    if(!_2s["date"]){
      _2s["date"] = (new Date()).getTime();
    }
    var _Valid = _Util.validations(vld,_2s);   
    if(_Valid.valid){
      _updFormObs();
      _2s["type"] = "TRANSFER";
      _2s["delivery"] = false;


      
      let trdest = JSON.parse(JSON.stringify(_2s));
      trdest["agentId"] = _2s["agentDestination"]
      trdest["agentDestination"] = _2s["agentId"]


     

      let trsender = JSON.parse(JSON.stringify(_2s));
      trsender["amount"] = trsender["amount"] *-1;



      console.log(trsender)

      console.log(trdest)


      let Qry = {
        form:trsender,
        fields:[
          "id","senderId","receiverId","agentId","amount","date","type"
        ],
        query:"upgradeMovements"
      };
      fetchRemesa(Qry,dispatch,_formName);

      let QryD = {
        form:trdest,
        fields:[
          "id","senderId","receiverId","agentId","amount","date","type"
        ],
        query:"upgradeMovements"
      };
      fetchRemesa(QryD,dispatch,_formName);


      _formName && _Util.updFormStore(_formName,{});
      props.history.push({pathname:"/remesas"});
    }
  }

  
  var _usersList = _state["usersList"];

  let _usersPrs = parseAutoObj(_usersList,"personName")


  let _usersDiffPrs = _form["agentId"] && parseAutoObjIgnore(_usersList,"personName",_form["agentId"]);

  const handleInputDate = (v,f) => {
    let frm =  _form;
    frm[f] = v.id;
    //_Util.updFormStore(_formName,frm)
    //validateFields()
  }



  
  const handleIsReseller = (v,k) => {
    let bdy = {
      query:"upgradeAgentsbyAdmin",
      form:{
        isReseller:k,
        id:v
      },
      params:{userId:"113699695841584167881"},
      Collection:"Users"
    };
    let itm = _usersList[v]
    console.log(itm)
    if(!itm["resellerSenderID"]){
      let _idSnd = _Util.gen16CodeId();
      let Qry = {
        form:{
          id:_idSnd,
          name:itm.personName,
          dispatcherId: itm.dispatcherId,
          email: itm.personEmail,
          resellerId: itm.id,
        },
        fields:['id','name',"phoneNumber","email","dispatcherId"],
        query:"upgradeSenders"
      };      
      bdy["form"]["resellerSenderID"] = _idSnd;
      fetchRemesa(Qry,dispatch);
    }
    fetchRemesa(bdy,dispatch);
  }

  const handleIsDispatcher = (v,k) => {
    let bdy = {
      query:"upgradeAgentsbyAdmin",
      form:{
        isDispatcher:k,
        id:v
      },
      params:{userId:"113699695841584167881"},
      Collection:"Users"      
    };
    fetchRemesa(bdy,dispatch);
  }


  const handleAgentByDispatcher = (v,k) => {
    let bdy = {
      query:"upgradeAgentsbyAdmin",
      form:{
        dispatcherId:k,
        id:v
      },
      params:{userId:"113699695841584167881"},
      Collection:"Users"
    };
    fetchRemesa(bdy,dispatch);
  }


  const handleCommissionPercent = (v,k) => {
    let bdy = {
      query:"upgradeAgentsbyAdmin",
      form:{
        commissionPercent:k*1,
        id:v
      },
      params:{userId:"113699695841584167881"},
      Collection:"Users"      
    };
    fetchRemesa(bdy,dispatch);
  }



  const handleTelegram_chat_id = (v,k) => {
    let bdy = {
      query:"upgradeAgentsbyAdmin",
      form:{
        telegram_chat_id:k,
        id:v
      },
      params:{userId:"113699695841584167881"},
      Collection:"Users"      
    };
    fetchRemesa(bdy,dispatch);
  }
  
  



  return (
      <>
        <style>
        {`

      .palette{
        --base-color: rgb(109, 76, 65);
        --base-color-gradient: 109, 76, 65;
      }

        `}
        </style>
        <div className={`mainViewHero palette formContainer  ${choosePayView === 1?'_payView':''} `} style={{opacity:view?1:0, minHeight:"1600px"}} >
          <div className={`loloHero`}> 
            <div className={`formHero _dsplFlx spaceAround`}> 
              <div className={`hdWrp`}> 
              <div className={`titleHero`} style={{minHeight:"60px"}}>
                  {` `}
                </div>
                <div className={`descHero`}>
                {` `}
                </div>
                <div className={`form`}> 
                <div className={`formContainer`} style={{opacity:1}}>
                  
                <div className={`formContainer  minHe`} style={{opacity:choosePayView === 0?1:0}}>
                  {choosePayView === 0?
                  <>
                  <div className={`formContainer`} style={{opacity:formView === 1?1:0}}>
                  {formView===1?
                  <div className={`pym81b sendBx `}>
                    <div className={`_labelBx`}>Link to Dispatcher</div>
                    <div className={`_dsplFlx spaceAround _flxWrp`}>
                      <div className={`paddField address`}>
                          <InputAutocomplete 
                            icon={`more_vert`} 
                            form={_formName} 
                            field={`agentId`}  
                            keyCode={16} 
                            background={`#f5f5f5`}
                            color={`var(--base-color)`}
                            placeholder={`Agent`} 
                            // OnChange={(e)=>handleInputReceive(e,`city`)}
                            validations={vldFlds2[`agentId`]}
                            initvalue={_form["agentId"]}
                            OnSelect={(e)=>handleInputAutoReceive(e,`agentId`)} 
                            data={_usersPrs}
                            //OnChange={(e)=>setGroupSearch(e,"name")}
                          />
                      </div>
                      <div className={`paddField address`}>
                          <InputAutocomplete 
                            icon={`more_vert`} 
                            form={_formName} 
                            field={`agentDestination`}  
                            keyCode={17} 
                            background={`#f5f5f5`}
                            color={`var(--base-color)`}
                            placeholder={`Dispatcher`} 
                            // OnChange={(e)=>handleInputReceive(e,`city`)}
                            validations={vldFlds2[`agentDestination`]}
                            initvalue={_form["agentDestination"]}
                            OnSelect={(e)=>handleInputAutoReceive(e,`agentDestination`)} 
                            data={_usersDiffPrs}
                            //OnChange={(e)=>setGroupSearch(e,"name")}
                          />
                      </div>
                      </div>
                      <div className={`_dsplFlx spaceAround _flxWrp`}>
                      <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleAgentByDispatcher(_form["agentId"],null )}>
                            <BTN_f theme={`fire_brick`} title={"remove Link"}/>
                          </span>
                        </div>
                      <div className={`fieldPadding _MrgV`}>                        
                        <span onClick={()=>handleAgentByDispatcher(_form["agentId"],_form["agentDestination"] )}>
                          <BTN_f theme={`light_blue`} title={"Link"}/>
                        </span>
                      </div>
                      <div className={`fieldPadding _MrgV`}>                        
                        <span onClick={()=>handleAgentByDispatcher(_form["agentId"],"113699695841584167881" )}>
                          <BTN_f theme={`light_blue`} title={"Link2Admin"}/>
                        </span>
                      </div>
                    </div>
                  </div>
                  :null}
                  </div>
                  <div className={`formContainer`} style={{opacity:formView === 1?1:0}}>
                  {formView===1?
                  <div className={`pym81b sendBx `}>
                    <div className={`_labelBx`}>Make Dispatcher</div>
                    <div className={`_dsplFlx spaceAround _flxWrp`}>
                      <div className={`paddField address`}>
                          <InputAutocomplete 
                            icon={`more_vert`} 
                            form={_formName} 
                            field={`agentId2`}  
                            keyCode={96} 
                            background={`#f5f5f5`}
                            color={`var(--base-color)`}
                            placeholder={`Agent`} 
                            // OnChange={(e)=>handleInputReceive(e,`city`)}
                            validations={vldFlds2[`agentId2`]}
                            initvalue={_form["agentId2"]}
                            OnSelect={(e)=>handleInputAutoReceive(e,`agentId2`)} 
                            data={_usersPrs}
                            //OnChange={(e)=>setGroupSearch(e,"name")}
                          />
                      </div>
                      </div>
                      <div className={`_dsplFlx spaceAround _flxWrp`}>
                        <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleIsDispatcher(_form["agentId2"],false)}>
                            <BTN_f theme={`fire_brick`} title={"remove Dispatcher"}/>
                          </span>
                        </div>
                        <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleIsDispatcher(_form["agentId2"],true)}>
                            <BTN_f theme={`light_blue`} title={"set Dispatcher"}/>
                          </span>
                        </div>
                      </div>
                  </div>
                  :null}
                  </div>
                  <div className={`formContainer`} style={{opacity:formView === 1?1:0}}>
                  {formView===1?
                  <div className={`pym81b sendBx `}>
                    <div className={`_labelBx`}>Make Reseller</div>
                    <div className={`_dsplFlx spaceAround _flxWrp`}>
                      <div className={`paddField address`}>
                          <InputAutocomplete 
                            icon={`more_vert`} 
                            form={_formName} 
                            field={`agentId3`}  
                            keyCode={36} 
                            background={`#f5f5f5`}
                            color={`var(--base-color)`}
                            placeholder={`Agent`} 
                            // OnChange={(e)=>handleInputReceive(e,`city`)}
                            validations={vldFlds2[`agentId3`]}
                            initvalue={_form["agentId3"]}
                            OnSelect={(e)=>handleInputAutoReceive(e,`agentId3`)} 
                            data={_usersPrs}
                            //OnChange={(e)=>setGroupSearch(e,"name")}
                          />
                      </div>
                      </div>
                      <div className={`_dsplFlx spaceAround _flxWrp`}>
                        <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleIsReseller(_form["agentId3"],false)}>
                            <BTN_f theme={`fire_brick`} title={"remove Reseller"}/>
                          </span>
                        </div>
                        <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleIsReseller(_form["agentId3"],true)}>
                            <BTN_f theme={`light_blue`} title={"set Reseller"}/>
                          </span>
                        </div>
                      </div>
                  </div>
                  :null}
                  </div>
                  <div className={`formContainer`} style={{opacity:formView === 1?1:0}}>
                  {formView===1?
                  <div className={`pym81b sendBx `}>
                    <div className={`_labelBx`}>Commission Percent </div>
                    <div className={`_dsplFlx spaceAround _flxWrp`}>
                      <div className={`paddField address`}>
                          <InputAutocomplete 
                            icon={`more_vert`} 
                            form={_formName} 
                            field={`agentId44`}  
                            keyCode={71} 
                            background={`#f5f5f5`}
                            color={`var(--base-color)`}
                            placeholder={`Agent`} 
                            // OnChange={(e)=>handleInputReceive(e,`city`)}
                            validations={vldFlds2[`agentId44`]}
                            initvalue={_form["agentId44"]}
                            OnSelect={(e)=>handleInputAutoReceive(e,`agentId44`)} 
                            data={_usersPrs}
                            //OnChange={(e)=>setGroupSearch(e,"name")}
                          />
                      </div>
                      <div className={`paddField`}>
                        <InputText 
                          icon={`more_vert`} 
                          form={_formName} 
                          field={`amount`}  
                          keyCode={49} 
                          background={`#f5f5f5`}
                          color={`#5e35b1`}
                          placeholder={`${_Util.translatetext(41)}`} 
                          OnChange={(e)=>handleInput(e,`amount`)}
                         // validations={vldFlds2[`amount`]}
                          initvalue={_form["amount"]}
                        />
                      </div>
                      </div>
                      <div className={`_dsplFlx spaceAround _flxWrp`}>                        
                        <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleCommissionPercent(_form["agentId44"],_form["amount"])}>
                            <BTN_f theme={`light_blue`} title={"set Reseller"}/>
                          </span>
                        </div>
                      </div>
                  </div>
                  :null}
                  </div>
                  <div className={`formContainer`} style={{opacity:formView === 1?1:0}}>
                  {formView===1?
                  <div className={`pym81b sendBx `}>
                    <div className={`_labelBx`}>Update telegram chatId</div>
                    <div className={`_dsplFlx spaceAround _flxWrp`}>
                      <div className={`paddField address`}>
                          <InputAutocomplete 
                            icon={`more_vert`} 
                            form={_formName} 
                            field={`agentId94`}  
                            keyCode={23} 
                            background={`#f5f5f5`}
                            color={`var(--base-color)`}
                            placeholder={`Agent`} 
                            // OnChange={(e)=>handleInputReceive(e,`city`)}
                            validations={vldFlds2[`agentId94`]}
                            initvalue={_form["agentId94"]}
                            OnSelect={(e)=>handleInputAutoReceive(e,`agentId94`)} 
                            data={_usersPrs}
                            //OnChange={(e)=>setGroupSearch(e,"name")}
                          />
                      </div>
                      <div className={`paddField`}>
                        <InputText 
                          icon={`more_vert`} 
                          form={_formName} 
                          field={`telegram_chat_id`}  
                          keyCode={21} 
                          background={`#f5f5f5`}
                          color={`#5e35b1`}
                          placeholder={`telegram chatId`} 
                          OnChange={(e)=>handleInput(e,`telegram_chat_id`)}
                         // validations={vldFlds2[`amount`]}
                          initvalue={_form["telegram_chat_id"]}
                        />
                      </div>
                      </div>
                      <div className={`_dsplFlx spaceAround _flxWrp`}>                        
                        <div className={`fieldPadding _MrgV`}>
                          <span onClick={()=>handleTelegram_chat_id(_form["agentId94"],_form["telegram_chat_id"])}>
                            <BTN_f theme={`light_blue`} title={"Update"}/>
                          </span>
                        </div>
                      </div>
                  </div>
                  :null}
                  </div>
                  <div style={{marginBottom:'65px'}}></div>                  
                    {formView===1 && validForm ? 
                    <div className={'_w100  _dsplFlx spaceAround'}  >
                      <MoreInfoButton title={`Save`}  theme={"purple"}  clickEvent={()=>_save()} />
                    </div>
                    :null}
                    </>
                    :null}
                  </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={`formHero_gradient`} />
          <div  className={`mainView2Info`} />
        </div>
      </>
  );
}  





export default withRouter(BrowseComponent)


function parseAutoObj(o,ky,i){
  let res = {}
  _Util.ObjectKeys(o).map(k=>{
    res[k] = {name:o[k][ky], id:k}
  })
  return res;
} 


function parseAutoObjIgnore(o,ky,i){
  let res = {}
  _Util.ObjectKeys(o).map(k=>{
    if(k!==i){
      res[k] = {name:o[k][ky], id:k}
    }
  })
  return res;
} 