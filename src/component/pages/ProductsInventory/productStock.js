...
import {  OpenWatchDialog,  OpenModal, getDispatch} 
from '../../../actions/common'
...
const UpdInventory = loadable(() => import('./updInventory'))
...
const SendersComponent = (props) => {
  ...
  useEffect(() => {  
    ...
    if(!initialize){
      ...
      */
      if(_Util.IsAdmin() || _state["allowInventory"]){          
          loadStockProduct();
          setTimeout(()=>{setView(true); window.scrollTo(0,0);},50);
                    ...
                    {view?
                      ...
                      <div  className={`  headerTtl `}>
                        ...
                        <div  className={` ${isMobile?"":" _dsplFlx"} `}>
                          <div className={`descTitle`}>
                            {_Util.translatetext(72)}
                          </div>
                          <div className={`flexSpace`}/>
                          {invTt?
                          <div className={`descTitle`}>
                            {`${_Util.translatetext(73)}: `}
                      ...
                      <div  className={` _dsplFlx `}>
                        ...
                        <NavLink  to={{pathname:"/edit_products"}} className=" u3mD2d xwW5Ce"   onClick={()=>handleAdd()} >
                          <BTNH theme={`light_blue`} title={_Util.translatetext(75)}/> 
                        </NavLink>
                        </div>
                        </div>
                      </div>
                      ...
                      <div className={` _search_MrgV  `} >
                        <SearchInput updChanges={searchInventory} placeholderCode={91}/>
                      </div>
