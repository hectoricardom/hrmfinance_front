...
const calcMov = async (dispatch,mm) => {
    ...
    _Util.ObjectKeys(mm).map((mv,ii)=>{
      ...
      if(qry || dateQ){
        ...
        let _ord = mm[mv]["orderId"] && mm[mv]["orderId"].toLowerCase();
...
const updLng = (v) => { 
    ...
    return (
      <>
        ...
        <div className={` palette formContainer ${1?"shopping_cart  _movements drivers_view":""} ${isMobile?"is_mobile":""}`} style={{opacity:view?1:0}} >
                ...
                <div className={`centerListCardProd `}> 
                  ...
                  <div className={`  headerTtl`}>
                      ...
                      <div className={`_dsplFlx  spaceAround _flxWrp`}>
                        <div className={`pym81b sendBx mov_type _dsplFlx activeBlue mxWdth ${viewTypeMov===3?"fillBack":""}`} onClick={()=>changeView(3)}>
                          {isMobile? <div className={`flexSpace`}/>:null}
                          <div className={`icon_mov`}>
                            <Icon2 name={`shopping_outline`} />
                          </div>
                          <div className={`mov_type_title `}>
                            {isMobile?"":_Util.translatetext(105)}
                          </div>
                          <div className={`flexSpace`}/>                            
                        </div>
                        <div className={`pym81b sendBx mov_type _dsplFlx activeRed mxWdth ${viewTypeMov===1?"fillBack":""}`} onClick={()=>changeView(1)}>
                        {isMobile? <div className={`flexSpace`}/>:null}
                          <div className={`icon_mov`}>
                            <Icon2 name={`timer_outline`} />
                          </div>
                          <div className={`mov_type_title `}>
                            {isMobile?"":_Util.translatetext(52)}
                          </div>
                          <div className={`flexSpace`}/>                            
                        </div>
                        <div className={`pym81b sendBx mov_type _dsplFlx complete mxWdth ${viewTypeMov===2?"fillBack":""}`} onClick={()=>changeView(2)}>
                          {isMobile? <div className={`flexSpace`}/>:null}
                          <div className={`icon_mov`}>
                            <Icon2 name={`success`} />
                          </div>
                          <div className={`mov_type_title `}>
                            {isMobile?"":_Util.translatetext(53)}
                          </div>
                          <div className={`flexSpace`}/>
                        </div>
                      </div>
                      ...
                      <div className={` sendBx scroll3Wrp `} >
                        ...
                        <div className={`formContainer `} style={{opacity:viewTypeMov === 3?1:0}}>
                        {viewTypeMov === 3?
                        <div>
                          <div className={`_dsplFlx`}>
                            ...
                            <div className={`fieldPadding _MrgV`} style={{ marginTop: "10px"}}>
                              <span onClick={()=>{}}>
                                <BTNH theme={`light_blue`} title={`Refresh`}/>
                              </span>
                            </div>
                          </div>
                          <div className={`_dsplFlx space2Around _flxWrp op4oU`}>
...
classTyp = isPaid?"_trnsfr":"_trnDEsfr";
