...
const updInventory = async (i) => {
  ...
  _2s["type"] = "INVESTMENT_INGREDIENTS";
  if(!_2s["delivery"]){
    _2s["delivery"] = true;
  }
  if(!_2s["amount"]){
  ...
  let QryShp = {
    query:"AddIngredientstoInventoryfromQvamarket",
    form:_2s,
    params:{
      userId:_userId
    }
...
const invTt = calcInv(_state["ingredientsList"]);
...
const searchInventory = (searchQ) => {
  if(searchQ && searchQ.length>1){
    let dt = _Util.parseInventory(_state["ingredientsList"],_categoriesList,searchQ);
    setList(dt);
  }else{
    let dt = _Util.parseInventory(_state["ingredientsList"],_categoriesList,null);
    setList(dt);
  }
}
...
let keyCodes =   _Util.getGlobalsKeys();
    ...
    return (
      <>
        ...
        <div className={` palette formContainer ingredientInv ${isMobile?"is_mobile":""}`} style={{opacity:view?1:0}} >
                      ...
                      <div  className={`  headerTtl `}>
                        ...
                        <div  className={` ${isMobile?"":" _dsplFlx"} `}>
                          ...
                          <div className={`descTitle`}>
                            {`Inventario Actual: `}
                            <span className={`InvTotal`}>{`$${invTt.toFixed(2)}`} </span>
                          </div>
                          :null}
                        </div>
                      </div>
                      ...
                      <div  className={isMobile?"":` _dsplFlx `}>
                        ...
                        <div className={`fieldPadding _MrgV _svInvt`}>
                          <span onClick={()=>updInventory()}>
                            <BTNH theme={`blue_white`} title={_Util.translatetext(74)}/>
                          </span>
                        </div>
                        :null}
                        <div className={`flexSpace`}/>
                        ...
                        <NavLink  to={{pathname:"/edit_ingredients"}} className=" u3mD2d xwW5Ce"   onClick={()=>handleAdd()} >
                          <BTNH theme={`light_blue`} title={`${_Util.translatetext(12)} ${`Ingrediente`}`}/> 
                        </NavLink>
                        </div>
                        </div>
                      </div>
...
const ItemProdComponent = (props) => {
  ...
  const productsList = _state["ingredientsList"] || {}; 
...
*/
  ...
  const inv2 = inv2upd[_id];


  let unitMeasurement = _state["unitMeasurement"]
  
  let _unit = (item && item["unit"] && unitMeasurement[item["unit"]]) || {}
              ...
              <div className="egZxgf pqv9ne">
                <div className="egZxgf pqv9ne">
                  <div className="DX0ugf ApBhXe _dsplFlx">
                    ...
                    <span className="unitlbl mrkPl">{_unit && `(${_unit.name})`}</span>
                    <div className={`flexSpace`}/>
                  </div>
                </div>
                <NavLink  to={{pathname:"/edit_ingredients",search:"?prodId="+ _id}} className=" u3mD2d xwW5Ce" onClick={handleDetails}>
                  <div className="MPhl6c pqv9ne azTb0d YAEPj SGmlof" title={item && `${item["name"]}` }>{item && `${item["name"]}` }</div>
                </NavLink>
              </div>
            </article>
            <div className="egZxgf pqv9ne">
            {inv2?<MsgAlert text={`Inversion: ${inv2.qty?inv2.qty:0} X $${inv2.price?inv2.price:0}`} theme={'red'}/>:null}
            </div>
            <div className="egZxgf pqv9ne">
              <div className={`_dsplFlx spaceAround _flxWrp `}>
                <div className={`fieldPadding _MrgV`}>
                ...
                <div className={`fieldPadding _MrgV`} style={{marginTop:isMobile?"":"10px"}}>
                  <NavLink  to={{pathname:"/ingredients_movements", search:"?prodId="+ _id}}>
