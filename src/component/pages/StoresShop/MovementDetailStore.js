...
const SendersComponent = (props) => {
  ...
  useEffect(() => {  
    ...
    if(!initialize){
      ...
      if(!(_Util.IsAdmin() || _state["isqvmAgent"])){
            ...
            <div className={`formContainer centerListCardProd `} style={{opacity:1}}>  
              <div className={` _flxWrp HRKRR`}>
                <div className={`  headerTtl`}>
                  ...
                  :null}
                  {_Util.IsAdmin() && hasReceiver && hasReceiver["phoneNumber"]?
                  <div className={`infoDesc`}>
                    <div className={`_key`}>
                  ...
                  :null}
                  {_Util.IsAdmin() && hasReceiver && hasReceiver["address"]?
                  <div className={`infoDesc `}>
                    <div className={`_key`}>                      
...
else if(mvD["type"]==="INVESTMENT_FOOD"){
  prc = item && item["price"];
