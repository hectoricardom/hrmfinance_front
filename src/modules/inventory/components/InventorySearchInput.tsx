...
import { inventoryStore, InventoryItem, InventoryMovement } from '../stores/inventoryStore';
import { useTranslation } from '../../../translations';

interface InventorySearchInputProps {
  onInventorySelect?: (inventory: InventoryMovement) => void;
...
const InventorySearchInput: Component<InventorySearchInputProps> = (props) => {
  const { t } = useTranslation();
  const [searchTerm, setSearchTerm] = createSignal('');
  const [loading, setLoading] = createSignal(false);
  const [error, setError] = createSignal<string | null>(null);
  const [searchResults, setSearchResults] = createSignal<InventoryMovement[]>([]);
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      ...
      try {
        ...
        props.onSearchResults?.(query);
      } catch (apiError) {
        console.warn('API search failed, using local fallback:', apiError);
  ...
  const handleInventorySelect = (inventory: Inventory) => {
    ...
    setFocusedIndex(-1);
    props.onInventorySelect?.(inventory);
  };
    ...
    switch (e.key) {
      ...
      case 'Enter':
        ...
        if (focusedIndex() >= 0 && searchResults()[focusedIndex()]) {
          handleInventorySelect(searchResults()[focusedIndex()]);
        }
        break;
      case 'Escape':
        setShowDropdown(false);
  ...
  const getCurrentStock = (inventoryId: string) => {
  ...
  const inventoryInfoStyle = {
    display: 'flex',
    'justify-content': 'space-between',
    'align-items': 'center'
  };

  const inventoryDetailsStyle = {
    flex: '1'
  };

  const inventoryNameStyle = {
    'font-weight': '500',
    ...
    <div style={containerStyle}>
      <div style={inputContainerStyle}>
        <input
          ...
          onFocus={handleFocus}
          onBlur={handleBlur}
          placeholder={props.placeholder || t('inventory.searchInventorys', 'Search inventorys by name, SKU, or category...')}
            ...
            fallback={
              searchTerm() && !loading() && (
                <div style={emptyStateStyle}>
                  {t('inventory.noInventorysFound', 'No inventorys found matching your search.')}
                </div>
              )
            }
          >
            <For each={searchResults()}>
              {(inventory, index) => {
                const currentStock = getCurrentStock(inventory.id);
                const isLowStock = currentStock <= inventory.minStockLevel;
                ...
                return (
                  ...
                  >
                    <div style={inventoryInfoStyle}>
                      <div style={inventoryDetailsStyle}>
                        <div style={inventoryNameStyle}>{inventory.name}</div>
                        <div style={inventoryMetaStyle}>
                          <span>{t('inventory.sku', 'SKU')}: {inventory.sku}</span>
                          <span>•</span>
                          <span>{inventory.category}</span>
                          <span>•</span>
                          <span>{formatCurrency(inventory.unitCost)}</span>
                        </div>
                      </div>
                      <div style={stockInfoStyle}>
                        <div style={{ 
                          ...
                          {currentStock} {inventory.unitOfMeasure}
                        </div>
