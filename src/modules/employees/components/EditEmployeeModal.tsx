...
const EditEmployeeModal: Component<EditEmployeeModalProps> = (props) => {
  ...
  createEffect(() => {
    if (props.employee) {
      setFormData({
        ...
        payType: props.employee.payType || 'salary',
        hourlyRate: props.employee.hourlyRate?.toString() || '',
        annualSalary: props.employee.annualSalary?.toString() || '',
        overtimeRate: props.employee.overtimeRate?.toString() || '1.5',
        standardHoursPerWeek: props.employee.standardHoursPerWeek?.toString() || '40',
        taxRate: props.employee.taxRate?.toString() || '',
        insuranceDeduction: props.employee.insuranceDeduction?.toString() || ''
      });
    }
  });
  ...
  const handleSubmit = (e: Event) => {
    ...
    if (!props.employee || !data.name || !data.position || !data.salary  || !data.department) {
    ...
    employeeStore.updateEmployee(props.employee.id, updates);
    props.onClose();
  };
  ...
  return (
    <Modal isOpen={props.isOpen} onClose={props.onClose} title={t('employees.editEmployee')}>
