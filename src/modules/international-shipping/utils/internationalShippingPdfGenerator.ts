 ...
 */
const generateBarcode128 = (text: string): string => {
...
export const generateInternationalShippingPDF = async (
  ...
  // Generate Code-128 barcode with invoice ID
  const barcodeDataUrl = generateBarcode128(shippingData.invoice);
  ...
  doc.text('YABA SUR CARGO', margin, currentY + 5);
  ...
  // Add Code-128 barcode below QR code
  if (barcodeDataUrl) {
    const barcodeWidth = 50; // Barcode width in mm
    const barcodeHeight = 15; // Barcode height in mm
    doc.addImage(barcodeDataUrl, 'PNG', pageWidth - margin - barcodeWidth - 21, currentY , barcodeWidth, barcodeHeight);
  }
  ...
  if (qrCodeDataUrl) {
    doc.addImage(qrCodeDataUrl, 'PNG', pageWidth - margin - qrSize - 8, currentY+3, qrSize, qrSize);
  }
  ...
  drawBox(margin + col1Width + col2Width, currentY, col4Width+col3Width, rowHeight, true);
  ...
  if (shippingData.shipmentItems && shippingData.shipmentItems.length > 0) {
    shippingData.shipmentItems.forEach((item, index) => {
      ...
      drawBox(margin + col1Width + col2Width, currentY, col4Width+col3Width, rowHeight);
