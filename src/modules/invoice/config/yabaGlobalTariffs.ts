...
export type AirShippingFrequency = 'weekly' | 'biweekly'; // semanal o quincenal

export interface AirShippingRates {
...
export interface YabaGlobalConfig {
  ...
  airShipping: {
    weekly: AirShippingRates;      // Semanal
    biweekly: AirShippingRates;    // Quincenal (cada 15 días)
    // Legacy support
    tiers?: AirShippingTier[];
    promotion?: ShippingPromotion;
  };
...
export const YABA_ITEM_TARIFFS: TariffCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'waterpump', name: 'Water Pump', nameEs: 'Turbina de Agua', price: 15 },
      { id: 'microwaveS', name: 'Small Microwave', nameEs: 'Microwave Pequeno', price: 4 },
      { id: 'microwaveB', name: 'Big Microwave', nameEs: 'Microwave Grande', price: 7 },
    ]
  },
  {
    id: 'electronics',
    ...
    items: [
      ...
      { id: 'generator', name: 'Generator', nameEs: 'Generador', price: 40 },
      { id: 'lithium_battery', name: 'Lithium Battery', nameEs: 'Bateria de Litio', price: 40 },
    ]
  },
  {
    id: 'refrigeration',
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'tires', name: 'Tires', nameEs: 'Gomas de Carro', price: 20 },
    ]
  },
  {
    id: 'parts_by_weight',
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'extraTariff', name: 'Extra Tariff', nameEs: 'Tarifas extras', price: 3 },
    ]
  },
];
...
export const YABA_AIR_SHIPPING: YabaGlobalConfig['airShipping'] = {
  // Semanal (Salida 3 días) - precios más altos por envío más rápido
  weekly: {
    tiers: [
      { minLbs: 11, maxLbs: 22.99, rangeLabel: '11-22 lbs', pricePerLb: 4.50 },
      { minLbs: 23, maxLbs: 55.99, rangeLabel: '23-55 lbs', pricePerLb: 3.99 },
      { minLbs: 56, maxLbs: 79.99, rangeLabel: '56-79 lbs', pricePerLb: 3.79 },
      { minLbs: 80, maxLbs: 86, rangeLabel: '80-86 lbs', pricePerLb: 3.29 },
    ],
    // No promotion for weekly
  },
  // Quincenal (Salida cada 15 días) - precios más económicos
  biweekly: {
    tiers: [
      { minLbs: 11, maxLbs: 30.99, rangeLabel: '11-30 lbs', pricePerLb: 3.40 },
      { minLbs: 31, maxLbs: 60, rangeLabel: '31-60 lbs', pricePerLb: 2.99 },
      //{ minLbs: 71, maxLbs: 86, rangeLabel: '71-86 lbs', pricePerLb: 2.49 },
      //{ minLbs: 87, maxLbs: Infinity, rangeLabel: '86+ lbs', pricePerLb: 2.49 },
    ],
    /*
    promotion: {
      threshold: 86,
      freeLbs: 10,
      description: '+86 lbs = 10 lbs GRATIS'
    }
    */
  },
  // Legacy support - defaults to biweekly rates
  /*
  tiers: [
    { minLbs: 11, maxLbs: 50, rangeLabel: '11-50 lbs', pricePerLb: 3.50 },
    { minLbs: 51, maxLbs: 70, rangeLabel: '51-70 lbs', pricePerLb: 2.99 },
    { minLbs: 71, maxLbs: 86, rangeLabel: '71-86 lbs', pricePerLb: 2.49 },
    { minLbs: 87, maxLbs: Infinity, rangeLabel: '86+ lbs', pricePerLb: 2.49 },
  ],
  promotion: {
 ...
 * @param frequency - 'weekly' (semanal) or 'biweekly' (quincenal/cada 15 días)
 */
export function calculateAirShippingCost(
  weightLbs: number,
  frequency: AirShippingFrequency = 'biweekly'
): {
  subtotal: number;
  freeLbs: number;
  ...
  frequency: AirShippingFrequency;
} {
  // Get rates based on frequency
  const rates = frequency === 'weekly' ? YABA_AIR_SHIPPING.weekly : YABA_AIR_SHIPPING.biweekly;
  const tiers = rates.tiers;
  const promo = rates.promotion;

  // Apply promotion if applicable
  const promotionApplied = promo && weightLbs > promo.threshold;
  const freeLbs = promotionApplied && promo ? promo.freeLbs : 0;
