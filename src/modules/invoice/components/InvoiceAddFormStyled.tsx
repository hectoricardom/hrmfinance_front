...
import { loadActiveOffers, calculatePrice as calculateOfferPrice, isOffersLoaded, getActiveConfig, switchActiveOffer, activeConfig } from '../../../services/activeOffersService';
import type { ShippingMethod, ItemCategory } from '../../../services/shippingOffersService';
import { yabaOffersApi } from '../../../services/apiAdapter';
...
interface InvoiceReserva {
  ...
  type: string;
  qty: number;
  weight: number; // Weight for misceláneas pricing calculation
  category: string; // Auto-categorized based on type
  price: number;
  arancel: number;
  total: number;
}
...
const DYNAMIC_PRICING = {
  ...
  MISCELANEAS: {
    over50weight: 2.99, // 50+ lbs get discount  
    under50weight: 3.99, // Less than 50 lbs
...
export const InvoiceAddForm = () => {
  ...
  const [autoCalculation, setAutoCalculation] = createSignal(true); // Toggle for auto-calculation


  const [selectedShipper, setSelectedShipper] = createSignal<any>(null);
  
  // Signal to track if offers are loaded
  ...
  onMount(async () => {
    ...
    // Load YABA offers configuration
    try {
      // Load all available offers
      const allOffers = await yabaOffersApi.getAll();
      ...
      devLog('✅ YABA Offers loaded for invoice:', config?.name);
    } catch (error) {
      console.warn('⚠️ Could not load YABA offers, using defaults');
      setOffersReady(true); // Still set ready to avoid blocking
    }
  ...
  // Handle offer configuration switch
  const handleOfferSwitch = async (offerId: string) => {
    try {
      setSuccess('🔄 Cambiando configuración...');
      await switchActiveOffer(offerId);
      setSuccess('✅ Configuración de ofertas actualizada');
      setTimeout(() => setSuccess(''), 3000);

      // Recalculate all reservas with new pricing
      const reservas = form().reservas;
      reservas.forEach(reserva => {
        const newPrice = calculateDynamicPrice(reserva.type, reserva.weight);
        const newTotal = reserva.weight * newPrice;
        updateReserva(reserva.id, { price: newPrice, total: newTotal });
      });
    } catch (error) {
      setError('Error al cambiar configuración de ofertas');
  ...
  const categorizeReservaType = (type: string): keyof typeof DYNAMIC_PRICING => {
    ...
    if (upperType.includes('TV') || upperType.includes('TELEFONO') || upperType.includes('PHONE') ||
  ...
  // 💰 CALCULAR PRECIO DINÁMICO BASADO EN CATEGORÍA Y PESO
  const calculateDynamicPrice = (reservaType: string, weight?: number): number => {
    const category = categorizeReservaType(reservaType);

    // Try to use loaded YABA offers first
    if (isOffersLoaded() && weight && weight > 0) {
      const shippingMethod: ShippingMethod = form().shippingMethod === 'AEREO' ? 'air' : 'maritime';
      ...
      const priceCalc = calculateOfferPrice(weight, shippingMethod, itemCategory);

      if (priceCalc) {
        devLog(`💰 Using YABA offer: ${weight}lbs @ $${priceCalc.pricePerLb}/lb = $${priceCalc.price}`);
    ...
    switch (category) {
      case 'MISCELANEAS':
        // Usar peso si está disponible
        if (weight && weight > 0) {
          return weight >= 50 ? DYNAMIC_PRICING.MISCELANEAS.over50weight : DYNAMIC_PRICING.MISCELANEAS.under50weight;
        }
        return DYNAMIC_PRICING.MISCELANEAS.under50weight;
      case 'DURADEROS':
  ...
  const calculateArancel = (reservaType: string, qty: number): number => {
    const category = categorizeReservaType(reservaType);
  ...
  const addReserva = () => {
    const newReserva = {
      ...
      arancel: 0,
      total: 0,
      autoCalculate: true // Default to auto-calculation enabled
    };
  ...
  // Update reserva with automatic pricing and categorization
  const updateReserva = (id: string, field: keyof InvoiceReserva, value: string | number) => {
    ...
    if (index !== -1) {
      ...
      // Auto-calculate when type, weight, or quantity changes
      if (field === 'type' && value && autoCalculation()) {
        const type = String(value);
        const category = categorizeReservaType(type);
        const dynamicPrice = calculateDynamicPrice(type, updated.weight);
        const dynamicArancel = calculateArancel(type, updated.qty);
        
        updated.category = category;
        updated.price = dynamicPrice;
        updated.arancel = dynamicArancel;
        updated.total = (dynamicPrice + dynamicArancel) * updated.qty;
      } 
      // Auto-calculate when weight changes (for misceláneas)
      else if (field === 'weight' && autoCalculation() && updated.type) {
        const category = categorizeReservaType(updated.type);
        if (category === 'MISCELANEAS') {
          const dynamicPrice = calculateDynamicPrice(updated.type, Number(value));
          updated.price = dynamicPrice;
          updated.total = (dynamicPrice + updated.arancel) * updated.qty;
        }
      }
      // Auto-calculate when quantity changes
      else if (field === 'qty' && autoCalculation() && updated.type) {
        // Recalculate price too (in case offer rates changed or weight-based tiers)
        const dynamicPrice = calculateDynamicPrice(updated.type, updated.weight);
        const dynamicArancel = calculateArancel(updated.type, Number(value));
        updated.price = dynamicPrice;
        updated.arancel = dynamicArancel;
        updated.total = (dynamicPrice + dynamicArancel) * Number(value);
      }
      // Manual recalculation when auto-calc is off or for manual price/arancel changes
      else if (field === 'qty' || field === 'price' || field === 'arancel') {
        updated.total = (Number(updated.price) + Number(updated.arancel)) * Number(updated.qty);
      }
  ...
  // Remove reserva with confirmation
  const removeReserva = (id: string) => {
    const index = form().reservas.findIndex(r => r.id === id);
    if (index !== -1) {
      const reserva = form().reservas[index];
      const confirmMessage = `¿Eliminar reserva "${reserva.type || 'sin tipo'}"?\n\nEsta acción no se puede deshacer.`;
    ...
    <div style={styles.container}>
      ...
      <Show when={offersReady() && isOffersLoaded()}>
        <div style={{
          ...
          <div>
            <strong>💰 Ofertas Activas:</strong> {getActiveConfig()?.name || 'Default'}
            <span style={{ 'margin-left': '1rem', opacity: 0.9 }}>
              ({form().shippingMethod === 'AEREO' ? '✈️ Aéreo' : '🚢 Marítimo'})
            </span>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', 'align-items': 'center' }}>
            <Show when={availableOffers().length > 1}>
              <button
                    ...
                    >
                      <div>
                        <div style={{ 'font-weight': '600' }}>
                          {isActive && '✓ '}
                          {offerData.offerName || 'Default'}
                        </div>
                        <div style={{ 'font-size': '0.75rem', color: 'var(--text-muted)', 'margin-top': '0.25rem' }}>
                          Código: {offerData.offerCode}
                        </div>
                      </div>
                      <Show when={isActive}>
                        <span style={{
        ...
        <Show when={form().shippingMethod && form().invoice && form().shipper_consignee?.cid && form().shipper_consignee?.name} >
           ...
           <div style={{ ...styles.section, 'margin-top': '1rem' }}>
            <div style={{ ...styles.sectionTitle, 'justify-content': 'space-between' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', 'align-items': 'center' }}>
                <button
                  onClick={() => setAutoCalculation(!autoCalculation())}
                  style={{ 
                    ...styles.button,
                    background: autoCalculation() ? 'var(--success-color)' : 'var(--error-color)',
                    color: 'white',
                    'font-size': '0.75rem',
                    padding: '0.5rem 0.75rem'
                  }}
                  title={autoCalculation() ? 'Click para desactivar cálculo automático' : 'Click para activar cálculo automático'}
                >
                  {autoCalculation() ? '🤖 Auto-Cálculo ON' : '✋ Auto-Cálculo OFF'}
                </button>
                <button
                  onClick={addReserva}
            ...
            <Show when={form().reservas.length > 0}>
              <div style={{
                ...
                <div style={{ 'font-weight': 'bold', 'margin-bottom': '0.5rem' }}>
                  💰 Precios Automáticos {autoCalculation() ? '(🤖 ACTIVO)' : '(✋ DESACTIVADO)'}
                </div>
                <div style={{ 'line-height': '1.4', color: 'var(--text-muted)' }}>
                  • <strong>Misceláneas:</strong> $3.99 (&lt;50 lbs) / $2.99 (≥50 lbs) - Calculado por peso<br/>
                  • <strong>Duraderos:</strong> $4.50 + arancel dinámico (0-5: $5, 5-20: $10, 20-50: $20, etc.)<br/>
                  • <strong>Exclusivos:</strong> $6.99 (TVs, computadoras - requieren confirmación)<br/>
                  • <strong>Baterías Litio:</strong> $8.99 (bulto exclusivo)<br/>
                  • <strong>Documentos:</strong> $1.99
                  {!autoCalculation() && (
                    <><br/><strong style={{ color: 'var(--error-color)' }}>⚠️ Auto-cálculo desactivado:</strong> Los precios y aranceles deben ingresarse manualmente</>
                  )}
                </div>
              </div>
            </Show>
...
const ReservaInvoice: Component<any> = (props) => {
  ...
  return (
    <div style={styles.productWrapItem}>
      <div style={{ ...styles.productItem, "justify-content": 'space-between', "align-items": 'center' }}>
        <div style={{ display: 'flex', 'align-items': 'center', gap: '0.5rem', flex: '1' }}>
          <span style={{ 'font-weight': 'bold', color: 'var(--primary-color)', 'font-size': '0.9rem' }}>
            📋 {reserva.type || 'Sin tipo'} 
          </span>
      ...
      <div style={styles.productItem}>
        ...
        <Show when={reserva.category === 'MISCELANEAS'}>
          <div style={styles.formGroup}>
            <label style={styles.label}>Peso (lbs)</label>
            <InputOnFor
              type="number"
              value={reserva.weight || 0}
              update={(value) => updateReserva(reserva.id, 'weight', parseFloat(value) || 0)}
              style={{ ...styles.input, ...styles.smallInput, width: '60px' }}
              placeholder="Lbs"
              min="0"
              step="0.1"
        ...
        <div style={styles.formGroup}>
          ...
          <InputOnFor
            ...
            style={{ 
              ...styles.input, 
              ...styles.smallInput,
              background: autoCalculation() ? 'var(--strip-color)' : 'var(--surface-color)',
              cursor: autoCalculation() ? 'not-allowed' : 'text'
            }}
            placeholder="Precio"
            step="0.01"
            index={3}
...
const InputOnFor: Component<any> = (props) => {
  ...
  // Create effect to update val when props.value changes
  createEffect(() => {
    if (props?.value !== undefined && props.value.toString() !== val()) {
      setVal(props.value.toString());
    }
  });
