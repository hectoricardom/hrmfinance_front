...
import { generateInvoiceProofPDF } from '../../../utils/invoiceProofPdfGenerator';
...
const InvoiceDisplay: Component<InvoiceDisplayProps> = (props) => {
  ...
  const [bagQrCodeUrls, setBagQrCodeUrls] = createSignal<Map<string, string>>(new Map());
  ...
  // Handle customer invoice proof PDF export
  const handleGenerateCustomerProof = async () => {
    devLog('Starting customer invoice proof PDF generation...');
    try {
      if (!props.invoice) {
        console.error('No invoice data available');
      ...
      // Prepare invoice data for the proof PDF
      const proofData = {
        ...
        products: props.invoice.products?.map(p => ({
          productName: p.product?.label || p.product?.code || 'Producto',
          qty: p.qty,
          salePrice: p.salePrice,
          total: p.total || p.qty * p.salePrice
        })),
        reservas: props.invoice.reservas?.map(r => ({
          type: r.type,
          qty: r.qty,
          price: r.price,
          arancel: r.arancel,
          total: r.total || (r.qty * r.price + r.arancel)
        })),
        services: props.invoice.services?.map(s => ({
          type: s.type,
          qty: s.qty,
          arancel: s.arancel,
          price: s.price
        })),
        yabaBulks: props.invoice.yabaBulks,
        yabaGrandTotal: props.invoice.yabaBulks?.reduce((sum, b) => sum + b.total, 0),
      ...
      await generateInvoiceProofPDF(proofData as any, {
        filename: `comprobante-${props.invoice.invoice}.pdf`
      });

      devLog('Customer invoice proof PDF generated successfully');
    } catch (error) {
      console.error('Error generating customer proof PDF:', error);
  ...
  // Generate QR codes for YABA-EXPRESS bag labels (encodes bag number)
  const generateBagQRCodes = async () => {
    const bulks = props.invoice.yabaBulks || props.invoice.bulks || [];
    if (!bulks.length) return;

    const urls = new Map<string, string>();
    for (const bulk of bulks) {
      try {
        const bagNumber = bulk.name || bulk.id;
        const qrUrl = await QRCode.toDataURL(bagNumber, {
          width: 200,
          margin: 1,
          color: { dark: '#000000', light: '#FFFFFF' }
        });
        urls.set(bulk.id || bulk.name, qrUrl);
      } catch (error) {
        console.error(`Error generating bag QR code for ${bulk.name}:`, error);
      }
    }
    setBagQrCodeUrls(urls);
  };

  // Generate YABA-EXPRESS 4x6 bag labels PDF
  const generateBagLabels = async () => {
    const bulks = props.invoice.yabaBulks || props.invoice.bulks || [];
    if (!bulks.length) {
      alert('No hay bultos para generar etiquetas');
      return;
    ...
    await generateBagQRCodes();

    try {
      const pdfWidth = 101.6;  // 4 inches in mm
      const pdfHeight = 152.4; // 6 inches in mm
      ...
      for (let i = 0; i < bulks.length; i++) {
        const bulk = bulks[i];
        const bagNumber = bulk.name || bulk.id;
        const qrUrl = bagQrCodeUrls().get(bulk.id || bulk.name);
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(0, 0, 0);
        pdf.text('YABA-EXPRESS', pdfWidth / 2, 25, { align: 'center' });
        ...
        if (qrUrl) {
          ...
          pdf.addImage(qrUrl, 'PNG', qrX, 42, qrSize, qrSize);
        }

        // Bag number — large text below QR
        pdf.setFontSize(28);
        pdf.setFont('helvetica', 'bold');
        const textLines = pdf.splitTextToSize(bagNumber, pdfWidth - 20);
        let currentY = 118;
        textLines.forEach((line: string) => {
          pdf.text(line, pdfWidth / 2, currentY, { align: 'center' });
          currentY += 12;
        });

        // Invoice reference at bottom
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(100, 100, 100);
        pdf.text(`Invoice: ${props.invoice.invoice}`, pdfWidth / 2, pdfHeight - 10, { align: 'center' });
      }

      const filename = `YABA-EXPRESS-labels-${props.invoice.invoice}-${Date.now()}.pdf`;
      pdf.save(filename);

      devLog(`✅ YABA-EXPRESS bag labels PDF generated: ${filename}`);
      alert(`PDF generado exitosamente con ${bulks.length} etiqueta(s) YABA-EXPRESS`);

    } catch (error) {
      console.error('Error generating YABA-EXPRESS labels:', error);
      alert(`Error al generar etiquetas YABA-EXPRESS: ${(error as Error).message}`);
    }
  };
  ...
  return (
    ...
    <Show when={!props.invoice?.bulks?.length}>
      ...
      <Show when={props.invoice?.yabaBulks && props.invoice.yabaBulks.length > 0}>
        <div style={sectionStyle}>
          {/* YABA Header */}
          <div style={{
            display: 'flex',
            ...
            <div style={{ display: 'flex', 'align-items': 'center', gap: '0.5rem' }}>
              ...
              <span style={{ 'font-size': '1.2rem', 'font-weight': '700' }}>YABA Global Express</span>
            </div>
            <div style={{ display: 'flex', gap: '1rem', 'font-size': '0.85rem' }}>
              <span style={{ background: 'rgba(255,255,255,0.2)', padding: '0.25rem 0.5rem', 'border-radius': '4px' }}>
                {props.invoice.yabaBulks.length} Bulto{props.invoice.yabaBulks.length > 1 ? 's' : ''}
              </span>
              <span style={{ background: 'rgba(255,255,255,0.2)', padding: '0.25rem 0.5rem', 'border-radius': '4px' }}>
                {props.invoice.yabaBulks.reduce((sum, b) => sum + b.weight, 0).toFixed(1)} lbs
              </span>
              <span style={{ 'font-weight': '700', 'font-size': '1.1rem' }}>
                {formatCurrency(props.invoice.yabaBulks.reduce((sum, b) => sum + b.total, 0))}
              </span>
            </div>
          </div>

          {/* YABA Bulks List */}
          <For each={props.invoice.yabaBulks}>
            {(bulk, index) => (
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', 'align-items': 'center', gap: '0.5rem' }}>
                    ...
                    <span style={{ 'font-weight': '600', color: '#1e40af' }}>{bulk.name}</span>
                    <span style={{
                      padding: '0.15rem 0.4rem',
                      background: bulk.shippingMethod === 'aereo' ? '#1d4ed8' : '#0369a1',
                      color: 'white',
                      'border-radius': '4px',
                      'font-size': '0.65rem',
                      'font-weight': '600'
                ...
                <div style={{ padding: '0.75rem 1rem' }}>
                  {/* Items Table */}
                  <Show when={bulk.items && bulk.items.length > 0}>
                    <div style={{ 'margin-bottom': '0.75rem' }}>
                      <div style={{ 'font-size': '0.75rem', 'font-weight': '600', color: '#64748b', 'margin-bottom': '0.35rem' }}>
                        📦 Artículos ({bulk.items.length})
                      </div>
                          ...
                          <For each={bulk.items}>
                            {(item) => (
                              <tr style={{ 'border-bottom': '1px solid #f1f5f9' }}>
                                <td style={{ padding: '0.35rem 0', color: '#374151' }}>{item.name}</td>
                                <td style={{ padding: '0.35rem 0', 'text-align': 'center', color: '#6b7280', width: '60px' }}>x{item.qty}</td>
                                <td style={{ padding: '0.35rem 0', 'text-align': 'right', color: '#6b7280', width: '70px' }}>${item.unitPrice.toFixed(2)}</td>
                                <td style={{ padding: '0.35rem 0', 'text-align': 'right', 'font-weight': '600', width: '80px' }}>{formatCurrency(item.total)}</td>
                              </tr>
                            )}
                          </For>
                        </tbody>
                      ...
                      <div style={{
                        ...
                        <span style={{ color: '#64748b' }}>Subtotal Artículos:</span>
                        <span style={{ 'font-weight': '600' }}>{formatCurrency(bulk.itemsTotal)}</span>
                      </div>
                    </div>
                  </Show>
                  ...
                  <div style={{
                    ...
                    <div>
                      <div style={{ color: '#64748b', 'font-size': '0.7rem' }}>Peso</div>
                      <div style={{ 'font-weight': '600' }}>{bulk.weight.toFixed(1)} lbs</div>
                    </div>
                    <div>
                      <div style={{ color: '#64748b', 'font-size': '0.7rem' }}>Tarifa</div>
                      <div style={{ 'font-weight': '600' }}>${bulk.shippingDetails?.pricePerLb?.toFixed(2) || '0.00'}/lb</div>
                    </div>
                    <Show when={bulk.shippingDetails?.freeLbs > 0}>
                      <div>
                        <div style={{ color: '#16a34a', 'font-size': '0.7rem' }}>🎉 Lbs Gratis</div>
                        <div style={{ 'font-weight': '600', color: '#16a34a' }}>{bulk.shippingDetails.freeLbs} lbs</div>
                      </div>
                    </Show>
                    <div>
                      <div style={{ color: '#64748b', 'font-size': '0.7rem' }}>Costo Envío</div>
                      <div style={{ 'font-weight': '600' }}>{formatCurrency(bulk.shippingCost)}</div>
                    </div>
                  </div>
                  ...
                  <Show when={bulk.services && bulk.services.length > 0}>
                    <div style={{ 'margin-top': '0.5rem' }}>
                      <div style={{ 'font-size': '0.75rem', 'font-weight': '600', color: '#16a34a', 'margin-bottom': '0.25rem' }}>
                        🛠️ Servicios ({bulk.services.length})
                      </div>
                      <div style={{ display: 'flex', 'flex-wrap': 'wrap', gap: '0.35rem' }}>
                        <For each={bulk.services}>
                          {(service) => (
                            <span style={{
                      ...
                      <div style={{
                        ...
                        <span style={{ color: '#16a34a' }}>Total Servicios:</span>
                        <span style={{ 'font-weight': '600', color: '#16a34a' }}>{formatCurrency(bulk.servicesTotal)}</span>
                      </div>
                    </div>
                  </Show>
                </div>
          ...
          <div style={{
            ...
            <div style={{ 'text-align': 'center' }}>
              <div style={{ 'font-size': '0.7rem', color: '#64748b' }}>Tarifa Artículos</div>
              <div style={{ 'font-weight': '700', color: '#1e40af' }}>
                {formatCurrency(props.invoice.yabaBulks.reduce((sum, b) => sum + b.itemsTotal, 0))}
              </div>
            </div>
            <div style={{ 'text-align': 'center' }}>
              <div style={{ 'font-size': '0.7rem', color: '#64748b' }}>Costo Envío</div>
              <div style={{ 'font-weight': '700', color: '#1e40af' }}>
                {formatCurrency(props.invoice.yabaBulks.reduce((sum, b) => sum + b.shippingCost, 0))}
              </div>
            </div>
            <div style={{ 'text-align': 'center' }}>
              <div style={{ 'font-size': '0.7rem', color: '#64748b' }}>Servicios</div>
              <div style={{ 'font-weight': '700', color: '#16a34a' }}>
                {formatCurrency(props.invoice.yabaBulks.reduce((sum, b) => sum + b.servicesTotal, 0))}
              </div>
            </div>
            <div style={{ 'text-align': 'center', background: 'white', 'border-radius': '6px', padding: '0.5rem' }}>
              <div style={{ 'font-size': '0.7rem', color: '#1e40af' }}>Total YABA</div>
      ...
      <div style={actionButtonsStyle}>
        ...
        {/* Customer Invoice Proof Button */}
        <button
          style={{...printButtonStyle, background: 'linear-gradient(135deg, #0891b2 0%, #0d9488 100%)'}}
          onClick={handleGenerateCustomerProof}
          onMouseEnter={(e) => e.currentTarget.style.opacity = '0.9'}
          onMouseLeave={(e) => e.currentTarget.style.opacity = '1'}
        >
          📄 Comprobante Cliente
        ...
        <Show when={(props.invoice?.yabaBulks && props.invoice.yabaBulks.length > 0) || (props.invoice?.bulks && props.invoice.bulks.length > 0)}>
          <button
            style={{...printButtonStyle, background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)'}}
            onClick={generateBagLabels}
            onMouseEnter={(e) => e.currentTarget.style.opacity = '0.9'}
            onMouseLeave={(e) => e.currentTarget.style.opacity = '1'}
          >
            🏷️ YABA-EXPRESS 4x6
