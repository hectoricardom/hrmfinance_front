...
import { getDocumentRequestByToken, markDocumentUploaded, getUploadProgress, getMissingRequiredDocuments, formatExpiration, isRequestExpired, uploadTaxDocument, updateDocumentRequestPublic } from '../services/taxPortalApi';
import {  processTaxDocument } from '../services/taxDocumentApi';
...
import { logsOnDev, devLog } from '../../../services/utils';
              ...
              <div style={documentListStyle}>
                <For each={ request()?.requestedDocuments}>
                  {(doc) => {
                    ...
                    logsOnDev.log('Document:', doc.type, 'Label:', doc.label, 'Is verification form?', isVerificationFormType(doc.type));
                              ...
                              <div style={formFieldsStyle}>
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onInput={(e) => updateVerificationField('zipCode', e.currentTarget.value)}
                                  />
                                </div>
                            ...
                            <Show when={!signatureSubmitted() && !request()?.clientSignature?.signedAt}>
                              <div style={formFieldsStyle}>
                                ...
                                <button
                                  style={{
                                    ...submitButtonStyle,
                                    opacity: (!signatureInfo().agreedToTerms || !hasSignature() || !signatureInfo().name) ? 0.5 : 1,
                                    cursor: (!signatureInfo().agreedToTerms || !hasSignature() || !signatureInfo().name) ? 'not-allowed' : 'pointer'
                                  }}
                                  onClick={handleSubmitSignature}
                                  disabled={!signatureInfo().agreedToTerms || !hasSignature() || !signatureInfo().name}
                                >
                            ...
                            <Show when={!pinSubmitted() && !request()?.clientSigningPin?.setAt}>
                              <div style={formFieldsStyle}>
                                ...
                                <button
                                  style={{
                                    ...submitButtonStyle,
                                    opacity: (!signingPin() || !confirmPin() || signingPin().length !== 5 || signingPin() !== confirmPin() || !hasPinSignature() || !pinSignerName()) ? 0.5 : 1,
                                    cursor: (!signingPin() || !confirmPin() || signingPin().length !== 5 || signingPin() !== confirmPin() || !hasPinSignature() || !pinSignerName()) ? 'not-allowed' : 'pointer'
                                  }}
                                  onClick={handleSubmitSigningPin}
                                  disabled={!signingPin() || !confirmPin() || signingPin().length !== 5 || signingPin() !== confirmPin() || !hasPinSignature() || !pinSignerName()}
                                >
                                  {getText('submitPin')}
                                </button>
                              </div>
